...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
      ...
      DecodedJWT jwt;
      try {
        jwt = JwtUtils.parseToken(token);
      } catch (TokenExpiredException ex) {
        // 处理过期场景
        return unauthorizedResponse(exchange, "令牌已过期（过期时间：" + ex.getExpiredOn() + "）");
      } catch (JWTVerificationException ex) {
        // 其他验证失败场景
        return unauthorizedResponse(exchange, "令牌验证失败：" + ex.getMessage());
      }
      ...
      addHeader(mutate, SecurityConstants.LOGIN_USER, jwt.getPayload());
      // 内部请求来源参数清除
...
//        return chain.filter(exchange.mutate().request(mutate.build()).build());
    ...
    private Mono<Void> unauthorizedResponse(ServerWebExchange exchange, String msg) {
        ...
        return ServletUtils.webFluxResponseWriter(exchange.getResponse(), HttpStatus.UNAUTHORIZED, msg);
    }
