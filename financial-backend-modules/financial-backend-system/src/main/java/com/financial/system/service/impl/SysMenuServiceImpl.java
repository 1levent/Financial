...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements ISysMenuService {
    ...
    public List<SysMenu> selectMenuList(SysMenu menu, Long userId) {
        ...
        if (SysUser.isAdmin(userId)) {
            menuList = menuMapper.selectList(new QueryWrapper<>(menu));
        } else {
    ...
    public List<RouterVo> buildMenus(List<SysMenu> menus) {
        ...
        for (SysMenu menu : menus) {
            ...
            if (StringUtils.isNotEmpty(cMenus) && UserConstants.TYPE_DIR.equals(menu.getMenuType())) {
                ...
                metaVo1.setTitle(menu.getMenuName());
                metaVo1.setLink(menu.getPath());
                metaVo1.setIcon(menu.getIcon());
...
//        menuTrees.stream().map(
//            menu ->{
//                TreeSelect treeSelect = new TreeSelect();
//                treeSelect.setId(menu.getMenuId());
//                treeSelect.setLabel(menu.getMenuName());
//                treeSelect.setChildren(menu.getChildren());
//            }
//        )
//        return menuTrees.stream().map().collect(Collectors.toList());
        return null;
    }
    ...
    public boolean checkMenuNameUnique(SysMenu menu) {
        ...
        //todo 没判空可能会报错
        query.eq("menu_name", menu.getMenuName());
        query.eq("parent_id", menu.getParentId());
        if (StringUtils.isNotNull(menu.getMenuId())) {
            query.ne("menu_id", menu.getMenuId());
        }
        SysMenu res = this.getOne(query);
